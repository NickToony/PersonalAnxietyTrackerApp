...
import com.team5.dialog.ScrollableDialogFragment;
            ...
            switch (position) {
                case 0:
                   ...
                   points = ((Session) getActivity().getApplication())
                            .getUserAccount().getRecordByDayAverage(first.getTimeInMillis(), last.getTimeInMillis());
                    mRedLine = lineGraph.addLine(Color.RED);
                    mBlueLine = lineGraph.addLine(Color.BLUE);

                    for (int i = 0; i < points.size(); i++) {
                        // add to graph
                        lineGraph.addPoint(mBlueLine, i,points.get(i).getAnxiety(), "Random string",points.get(i).getTimestamp());
                        lineGraph.addPoint(mRedLine, i, points.get(i).getSeriousness(), "Random string", points.get(i).getTimestamp());
                    }
                ...
                case 1:
                    ...
                    points = ((Session) getActivity().getApplication())
                            .getUserAccount().getRecordByMonthAverage(c7.getTimeInMillis(), c8.getTimeInMillis());
                ...
                case 2:
                    ...
                    points = ((Session) getActivity().getApplication())
                            .getUserAccount().getRecordByYearAverage(c.getTimeInMillis(), c2.getTimeInMillis());
                    lineGraph.clearRecords();
                    mRedLine = lineGraph.addLine(Color.RED);
                    mBlueLine = lineGraph.addLine(Color.BLUE);
